...
public class BoneCPDBPool implements IDBPool {
	 ...
	 *            此连接池允许建立的最大连接数
	 */
	public BoneCPDBPool(PropertiesConfig mainConfig) throws Exception {
		Class.forName(mainConfig.getString("mysql-jdbc-driver")).newInstance();
		config = new BoneCPConfig();
		String mysqlUrl = String.format(mainConfig.getString("mysql-url"), mainConfig.getString("mysql-ip"), 
				mainConfig.getString("mysql-port"), mainConfig.getString("mysql-dbname"));
		config.setJdbcUrl(mysqlUrl);
		config.setUsername(mainConfig.getString("mysql-username"));
		config.setPassword(mainConfig.getString("mysql-password"));
		// 分区数 ，默认值2，最小1，推荐3-4，视应用而定
		config.setPartitionCount(mainConfig.getInt("mysql-fallow", 1));
		// 每个分区最小的连接数
		config.setMinConnectionsPerPartition(mainConfig.getInt("mysql-minconn", 5));
		// 每个分区最大的连接数
		config.setMaxConnectionsPerPartition(mainConfig.getInt("mysql-maxtotal", 10));
...
//		config.setReleaseHelperThreads(3);
		...
		config.setStatementsCacheSize(mainConfig.getInt("mysql-cachesize", 50));
